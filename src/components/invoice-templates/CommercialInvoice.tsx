...
export function CommercialInvoice({ data }: CommercialInvoiceProps) {
  return (
    <InvoiceLayout title="Commercial Vendor Tax Invoice">
      {/* Buyer Details */}
      <InvoiceSection title="Buyer Details">
        <FieldRow label="Business Name" value={data.buyer.name} />
        <FieldRow label="Business Address" value={data.buyer.address} />
      ...
      <InvoiceSection title="Invoice Price">
        <FieldRow label="Purchase Price (ex GST)" value={formatCurrency(data.invoice.purchasePrice)} />
      ...
      <InvoiceSection title="Disbursement Details">
        ...
        <TwoColumnFields
          fields={[
            ...
            { label: 'Bank', value: data.disbursement.bankAccount.bank },
        ...
        {data.disbursement.payoutBank.accountName && (
          <>
            <div className="text-xs font-semibold mb-1 mt-2">Payout Bank</div>
            <TwoColumnFields
              fields={[
                { label: 'Account Name', value: data.disbursement.payoutBank.accountName },
                { label: 'BSB', value: data.disbursement.payoutBank.bsbNumber },
                { label: 'Account Number', value: data.disbursement.payoutBank.accountNumber },
                { label: 'Amount', value: formatCurrency(data.disbursement.payoutBank.amount) },
              ]}
            />
          </>
        )}
